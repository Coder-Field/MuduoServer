...
typedef std::set<string> ConnectionSubscription;  //别名
...
class Topic : public muduo::copyable
{
 public:
  ...
  void add(const TcpConnectionPtr& conn)
  {
    ...
    if (lastPubTime_.valid()) //判断最好的发布时间是有效的
    {
      conn->send(makeMessage());  //订阅时 将最后一次发布有效的信息直接发送
    }
  }
...
class PubSubServer : noncopyable
{
 public:
      ...
      server_(loop, listenAddr, "PubSubServer")
  {
    ...
    loop_->runEvery(1.0, std::bind(&PubSubServer::timeP ublish, this));
  }
 ...
 private:
                   ...
                   const string& topic)
  {
    ...
    connSub->insert(topic);  //当前连接中保存topic， 用于后面连接断开 取消订阅的主题
    getTopic(topic).add(conn);  //将订阅当前 topic的连接加入到 topic中进行保存
  }
  ...
  EventLoop* loop_;
  TcpServer server_;
  std::map<string, Topic> topics_;  //hub端保存topic，当sub端订阅时，pub发布时，可遍历订阅的主题发送
};
...
int main(int argc, char* argv[])
{
  if (argc > 1)
  {
    ...
    pubsub::PubSubServer server(&loop, InetAddress(port));   //服务器，监听指定端口
    server.start();
    loop.loop();
  }
  else
